...
async function main() {
  mongoose.connect(
    "mongodb+srv://bhargavimachhi:Pranay12@cluster0.wzx3aup.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
  );
}
...
app.get("/appointments", async (req, res) => {
  ...
  for (let data of docData) {
    ...
    for(let d of data.appoinments) {
      d["doctor"] = data.username;
      doc.push(d);
    }
...
app.post("/appointments/:email/check", async (req, res) => {
  let doc = await doctor.findOne({ email: req.params.email });
  let docData = await doctor.find({});

  doc.appoinments.push({
    name: req.body.name,
    msg: req.body.msg,
  ...
  res.render("viewAppoinment.ejs", { doc: docs, docData });
});
